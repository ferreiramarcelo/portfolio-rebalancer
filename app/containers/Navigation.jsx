...
const Navigation = ({tab, user, changeTab, logOut}) => {
  const logInTab = (<Tab value="login" containerElement={ <Link to="/login" /> } label={ 'LOG IN' } />);

  const registerTab = (<Tab value="register" containerElement={ <Link to="/register" /> } label={ 'REGISTER' } />);

  const logOutButton = (<FlatButton onTouchTap={ logOut } label={ 'LOG OUT' } className={ cx('log-out-button') } />);

  const unauthenticatedTabs = (<Tabs value={ tab } onChange={ changeTab }>
                                 <Tab value="" label="Portfolio Rebalancer" containerElement={ <Link to="" /> } className={ cx('portfolio-rebalancer-tab') } />
  ...
  <div className={ cx('authenticated-container') }>
    <div className={ cx('authenticated-container-tabs') }>
      <Tabs value={ tab } onChange={ changeTab }>
        <Tab value="" label="Portfolio Rebalancer" containerElement={ <Link to="" /> } className={ cx('portfolio-rebalancer-tab') } />
        <Tab value="about" label="ABOUT" containerElement={ <Link to="/about" /> } />
        <Tab value="account" label="ACCOUNT" containerElement={ <Link to="/account" /> } />
      </Tabs>
    </div>
